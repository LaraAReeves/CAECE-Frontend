...
export default function Mapa(){
    ...
    const aulasTotales = [
        ...
        { id: 4, nombre: 'Aula 202', detalle: 'Recursos Humanos', piso: 1, profesor: 'Juan Perez', edificio: 'A' }, 
        { id: 5, nombre: 'Aula 203', detalle: 'Recursos Humanos', piso: 2, profesor: 'Juan Perez', edificio: 'A' }, 
        { id: 6, nombre: 'Aula 302', detalle: 'Recursos Humanos', piso: 1, profesor: 'Juan Perez', edificio: 'A' },
        { id: 7, nombre: 'Aula 301', detalle: 'Recursos Humanos', piso: 7, profesor: 'Juan Perez', edificio: 'A' } 
      ]
    ...
    useEffect(() => {
        ...
        if (busqueda) {
            const aulaEncontrada = aulasTotales.find(aula => aula.nombre === busqueda);
            if (aulaEncontrada) {
    ...
    return(
        ...
        <div className="flex flex-row justify-between w-screen px-5">
            ...
            {aulas.map((aula) => (
                <Aula aula={aula} key={aula.id} onClick={() => setAulaSeleccionada(aula)} className={ aulaSeleccionada?.nombre == aula.nombre ? "border-foreground" : "box-border"}>{aula.nombre}</Aula>
            ))}
            </div>
